...
def pixel2camera(camera_num, pixel_x, pixel_y):
    path_model = '/mnt/disk2/vsa/kdy/space-time/model_IDS/location/'
...
def my_loop():
    ...
    results_path='/mnt/disk2/vsa/VSA_test2/camera/txt_results/'
    times_path='/mnt/disk2/vsa/VSA_test2/camera/time/'
    #save_dir='/home/kdy/skdy/IDS/experiment/pixel_2_map/save_dir'
    #save_dir='/mnt/disk2/vsa/kdy/save_dir/'
    save_dir='/mnt/disk2/vsa/VSA_test2/camera/txt_results'
    ...
    # files 表示该文件夹下的文件list
        ...
        if results is None:
            ...
            camera_id=int(results.group(1))#如果没有匹配成功，应该直接跳过，而非报错。(应对那个detect和space-time)
        #print(camera_id)
        #get_location_from_txt(camera_num, results_txt_path, trail_img_path, flag_draw_trail=False, flag_save_plt=True)
        save_dir_path=os.path.join(save_dir,'space-time'+str(camera_id))
        if os.path.exists(save_dir_path):
            os.makedirs(save_dir_path)
        save_txt_path=os.path.join(save_dir_path,result_txt)
        time_txt_path=os.path.join(times_path,str(camera_id),result_txt)
        ...
        remove_old_file(save_dir_path,20)
        end=time.time()
