...
from space_time.new_space_time import space_time
import numpy as np
...
###
sys.path.append("./insightface/pythonpackage")
import insightface
import final_test as face
from insightface.app import FaceAnalysis

rpath = os.getcwd()
sys.path.insert(0, "persondetect/yolov5")
...
def initFaceModel():
    ...
    handler = insightface.model_zoo.get_model(
        '/home/vsa/pyproject/VSA_Server/insightface/pythonpackage/w600k_r50.onnx')
    handler.prepare(ctx_id=0)
    gallery_path = '/home/vsa/pyproject/VSA_Server/insightface/pythonpackage/gallery'
    gallery_x = np.zeros((309, 512), dtype=np.float32)  # 这里要修改大小
    for root2, num2, files2 in os.walk(gallery_path):
        if files2 != []:
            for fname2 in files2:
                num = int(fname2.rstrip('.jpg'))
                img2 = cv2.imread(os.path.join(root2, fname2))
                feat = handler.get_feat(img2)
                feat = feat / norm(feat)
                gallery_x[num] = feat
    return app,handler,gallery_x
...
class Worker(Process):
    ...
    def run(self):
        ...
        while True:
            ...
            path = origin_list[-2] # -1 所对应的最后一段视频还没有写完，所以分析导数第二个视频
            ...
            if not os.path.exists(self.person_mot_txt_path):
                os.makedirs(self.person_mot_txt_path)

            det_txt_path = self.person_detect_txt_path + video_name + '.txt'
            mot_txt_path = self.person_mot_txt_path+video_name+'.txt'
            ...
            if os.path.exists(det_txt_path):
                ...
                app, handler, gallery_x=initFaceModel()
                face.face_recognize(input_arr, mot_txt_path,self.camera_id,app,handler,gallery_x)
                f = open(mot_txt_path, 'r')
                lines = f.readlines()
                end_face_rec = time.time()
                with open('facerectime', 'a+') as f:
                   res = str(end_face_rec - start_face_rec) + ' ' + str(len(lines)) + '\n'
                   f.write(res)
                print("The face_recognize process takes ", str(end_face_rec - start_face_rec), " seconds!")
                
                print("================ face recognize done ===========")

                space_time(video_name + '.txt', self.camera_id)
                if self.write_flag:
...
if __name__ == "__main__":
    ...
    worker10 = Worker(28, 1)
    worker10.start()

    worker11 = Worker(35, 1)
    worker11.start()

    worker12 = Worker(36, 1)
    worker12.start()
